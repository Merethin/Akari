...
pub fn generate_processor_map() -> HashMap<&'static str, Processor> {
    ...
    map.insert("secelect", vec![BucketOrigin, Receptor(1)].into());
    // bucket: zombie
    ...
    map.insert("rscomply", vec![BucketOrigin, Actor(1), Data(vec![2])].into());
    ...
    map.insert("trcreate", vec![BucketOrigin, Actor(1), Data(vec![2,3])].into());
